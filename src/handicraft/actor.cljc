(ns handicraft.actor
  "HandicraftPracticeActor — the ISCO-08 7318 community handicraft actor as a
  `langgraph.graph/state-graph` (ADR-2607011000 / CLAUDE.md Actors
  section). One graph run = one handicraft operation request
  ...
  The unconditional invariant: the HandicraftPracticeAdvisor can never
  directly commit a record the HandicraftPracticeGovernor refuses —
  every commit-record! call is gated behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [handicraft.advisor :as advisor]
            [handicraft.governor :as governor]
            [handicraft.store :as store]))

(defn build-graph
  "Build a compiled HandicraftPracticeActor graph. `store` implements
  `handicraft.store/Store`. `advisor` implements
  `handicraft.advisor/Advisor` (defaults to `mock-advisor`).
